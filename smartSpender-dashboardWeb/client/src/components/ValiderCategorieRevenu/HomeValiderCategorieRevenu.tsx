...
export default function HomeValiderCategorieRevenu() {
  ...
  const handleValider = async (id) => {
    try {
      await axiosPrivate.put(
        `/suggestions-categories-revenu/${id}`,
  ...
  const handleDelete = async (id) => {
    try {
      await axiosPrivate.delete(`/suggestions-categories-revenus/${id}`);
      setCategories(
            ...
            <div id="table-button">
              <div class="table-responsive small">
                <div className="d-flex flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                  <Menu />
                  <h1 className="h2">Validation des categories de revenu</h1>
                </div>
                {/* <div id="contentt"> */}
                <main
                  role="main"
